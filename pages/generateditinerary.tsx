...
export default function GeneratedItinerary() {
    ...
    return (
      <div className="mt-0.5">
        <div className="flex items-center gap-4 mt-2">
          <button
            onClick={() =>
              setShowHours(prev => ({ ...prev, [business.id]: !prev[business.id] }))
            }
  ...
  const matchesUserPrefs = (business: any, prefs: string[]) => {
    ...
    return prefs.some(pref =>
      titles.some((title: string) => title.includes(pref.toLowerCase())) // ✅ fixed
    );
  };

  const pickStrict = (include: string[], prefs: string[], ...excludeGroups: string[][]) => {
  ...
  const generateRandom = () => {
    ...
    if (selectedCategories.food) {setFood(pickStrict(foodAliases, userPrefs.food, activityAliases, placeAliases));}
    if (selectedCategories.activity) {setActivity(pickStrict(activityAliases, userPrefs.activities, foodAliases, placeAliases));}
