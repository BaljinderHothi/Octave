...
export default function GeneratedItinerary() {
              ...
              <li key={day} className="flex">
                ...
                <span>
                  {entries
                    .map(
                      (e, idx) =>
                        `${formatTime(e.start)} – ${formatTime(e.end)}${e.is_overnight ? ' (Overnight)' : ''}`
                    )
                    .join(', ')}
                </span>
              </li>
  ...
  const matchesUserPrefs = (business: any, prefs: string[]) => {
    ...
    const titles =
      business.categories?.map((c: any) => `${c.title} ${c.alias}`.toLowerCase()) || [];
  ...
  const pickStrict = (
    ...
    const filtered = businessList.filter(
      (b) => isStrictlyInCategory(b, include, excludeGroups) && matchesUserPrefs(b, prefs)
    );
    if (filtered.length === 0) {
      const categoryMatches = businessList.filter((b) => isStrictlyInCategory(b, include, excludeGroups));
          ...
          <div className="flex-1">
            ...
            {item.rating && (
              <div className="mt-2 text-sm flex items-center gap-2">
                ...
                {item.review_count && <span className="text-gray-500">• {item.review_count} Yelp reviews</span>}
              </div>
            )}
        ...
        <div className="flex flex-col items-start mb-6">
            ...
            <div className="flex items-center">
              ...
              <div className="relative ml-1 group">
                ...
                <div className="absolute z-10 bg-white p-2 rounded shadow-lg w-64 text-xs opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                  Only using preferences from your initial profile setup. Additional preferences coming from your
                  reviews are not used for this feature (yet).
                </div>
              </div>
            </div>
            <div className="mt-1 space-y-1">
              {userPrefs.food.length > 0 && <p>• Food: {userPrefs.food.join(', ')}</p>}
              {userPrefs.activities.length > 0 && <p>• Activities: {userPrefs.activities.join(', ')}</p>}
              {userPrefs.places.length > 0 && <p>• Places: {userPrefs.places.join(', ')}</p>}
              {userPrefs.food.length === 0 &&
                userPrefs.activities.length === 0 &&
                userPrefs.places.length === 0 && <p className="italic">No preferences set - showing general recommendations</p>}
            </div>
          </div>
          ...
          {selectedLocation && (
            <div className="mt-2 text-sm text-gray-600 flex items-center">
              ...
              <span>Filtered to near the selected location</span>
            </div>
          )}
        ...
        <div className="space-y-1">
                ...
                <div className="w-8 h-8 rounded-full bg-red-100 flex items-center justify-center">
                  <img
                    ...
                    onError={(e: any) => {
                      ...
                      e.target.src =
                        "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%23ef4444' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'%3E%3Cpath d='M18 8h1a4 4 0 0 1 0 8h-1'/%3E%3Cpath d='M2 8h16v9a4 4 0 0 1-4 4H6a4 4 0 0 1-4-4V8z'/%3E%3Cline x1='6' y1='1' x2='6' y2='4'/%3E%3Cline x1='10' y1='1' x2='10' y2='4'/%3E%3Cline x1='14' y1='1' x2='14' y2='4'/%3E%3C/svg%3E";
                    }}
                  />
                </div>
              }
              onGenerate={() =>
                setFood(pickStrict(filteredBusinesses, foodAliases, userPrefs.food, activityAliases, placeAliases))
              }
            />
          )}
          ...
          {selectedCategories.activity && (
            <ItineraryCard
                ...
                <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center">
                  <img
                    ...
                    onError={(e: any) => {
                      e.target.onerror = null;
                      e.target.src =
                        "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%230ea5e9' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'%3E%3Ccircle cx='12' cy='12' r='10'/%3E%3Cpath d='M12 6v6l4 2'/%3E%3C/svg%3E";
                    }}
                  />
                </div>
              }
              onGenerate={() =>
                setActivity(
                  pickStrict(filteredBusinesses, activityAliases, userPrefs.activities, foodAliases, placeAliases)
                )
              }
            />
          )}
                ...
                <div className="w-8 h-8 rounded-full bg-green-100 flex items-center justify-center">
                  <img
                    ...
                    onError={(e: any) => {
                      e.target.onerror = null;
                      e.target.src =
                        "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%2322c55e' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'%3E%3Cpath d='M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z'/%3E%3Ccircle cx='12' cy='10' r='3'/%3E%3C/svg%3E";
                    }}
                  />
                </div>
              }
              onGenerate={() =>
                setPlace(pickStrict(filteredBusinesses, placeAliases, userPrefs.places, foodAliases, activityAliases))
              }
            />
          )}
        </div>
